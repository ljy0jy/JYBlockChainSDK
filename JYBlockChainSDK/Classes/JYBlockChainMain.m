...
@implementation JYBlockChainMain

static inline BOOL BTCMnemonicIsBitSet(uint8_t* buf, int bitIndex) {
    //uint8_t是8位的 所以除以8 转int 变成4字节32位了
    int val2 = ((int) buf[bitIndex / 8]);
    //所以val2是32位的 8位有效数，低8位。
    //&FF可以保证低八位有效，高位全部置0
    int val = val2 & 0xFF;
    //左移对8取余的位数，等于把要比较的那个位移动到 8位之中的最高位
    val = val << (bitIndex % 8);
...
static inline NSUInteger BTCMnemonicIntegerFrom11Bits(uint8_t* buf, int bitIndex) {
    NSUInteger value = 0;
    //从高位到低位， 遍历11位
    //如果为1则value左移 并 和0x01或（意思就是左移末尾置1）
    //如果不是1，则直接左移，末尾默认是置0的
    for (int i = 0; i < 11; i++) {
        //说白了就是判断当前位数是否是1， 是的话则左移置1
        //不是的话直接左移，具体操作看BTCMnemonicIsBitSet()里面的注释
...
+(NSString *)generateMnemonicString:(EntropyLenth)entropyLenth {
    if(entropyLenth % 32 != 0){
        [NSException raise:@"entropyLenth must be divisible by 32" format:@"entropyLenth Was: %ld",entropyLenth];
    }
    //创建比特数组
    NSMutableData *bytes = [NSMutableData dataWithLength:(entropyLenth/8)];
    //生成随机data
    int status = SecRandomCopyBytes(kSecRandomDefault, bytes.length, bytes.mutableBytes);
    //如果生成成功
    if(status == 0){
        return [[self class] mnemonicStringFromRandomData:bytes];
    }
    else{
...
+ (NSString *)mnemonicStringFromRandomData:(NSData *)entropy
{
    ...
    //把checkSum加到熵末尾 data的长度1 即8位 8位中，只有有效位数的值存在，其他位已被置0
    //这样不会影响checkSum的准确度
    NSMutableData* buf = [entropy mutableCopy];
    [buf appendBytes:&checksumByte length:1];
    
    //每11位 得到一个词，不能用buf.length，因为checksum有效可能不是8 ，但是data最低计量单位是8位
...
+ (NSData *)generateSeedFromMnemonicString:(NSString *)mnemonicString passphrase:(NSString *)passphrase {
    //这里简单点，就不多说了， 大致就是pbkdf2加密一下就有了seed
    passphrase = passphrase ?: @"";
    NSData* salt = [[@"mnemonic" stringByAppendingString:passphrase] dataUsingEncoding:NSUTF8StringEncoding];
    unsigned int  iterations = 2048;
   
    if (mnemonicString && ![mnemonicString isEqualToString:@""]) {
        NSData *dataKey = [mnemonicString dataUsingEncoding:NSUTF8StringEncoding];
        NSMutableData *seed = JYPDF2SHA512(dataKey, salt, iterations);
        return seed;
    }
    return nil;
}
...
+ (JYBlockChainKey *)generateKeyfromSeed:(NSData *)seed {
    NSData *data = [@"Bitcoin seed" dataUsingEncoding:NSUTF8StringEncoding];
    NSMutableData *outData = JYHMACSHA512(data, seed);
    
    JYBlockChainKey *blockChainKey = [JYBlockChainKey new];
    blockChainKey.privateKey = [NSData dataWithBytes:outData.bytes length:32];
    blockChainKey.chainCode = [NSData dataWithBytes:outData.bytes+32 length:32];
    //获取压缩公钥
    blockChainKey.compressedPublicKey = [blockChainKey compressedPublicKey];
...
+ (JYBlockChainKey *)generateWalletAddressFromMnemonicString:(NSString *)mnemonicString path:(NSString *)path{
    NSData *seed = [[self class] generateSeedFromMnemonicString:mnemonicString passphrase:@""];
    JYBlockChainKey *jykey = [[self class] generateKeyfromSeed:seed];
    return [jykey derivedKeychainWithPath:path];
}
